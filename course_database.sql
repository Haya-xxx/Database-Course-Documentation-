...
SELECT CourseID, COUNT(DISTINCT StudentID)
FROM Enrollments
GROUP BY CourseID;
...
SELECT AVG(DATEDIFF(DAY, EnrollDate, GETDATE()))
FROM Enrollments
WHERE CompletionPercent = 100;
--9. Percentage of students who completed each course. 
SELECT CourseID, COUNT(CASE WHEN CompletionPercent = 100 THEN 1 END), COUNT(*)
FROM Enrollments
GROUP BY CourseID;
--10. Count courses published each year.
SELECT YEAR(PublishDate), COUNT(*)
FROM Courses
